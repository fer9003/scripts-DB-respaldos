...
PGPASSWORD="colocarpassworddeBD" pg_dump --format=c -h endpoint -U dbuser databasename > /bacula/backups_prod/odooec/backup_${D}/backup_databasename_`date +%Y%m%d_%H%M`.sql
